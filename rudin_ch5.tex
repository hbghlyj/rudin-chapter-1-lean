...
(i) Since $f'$ is continuous on the compact interval $[a,b]$, it is uniformly continuous there.  Choose $\delta>0$ so that $|u-v|<\delta$ implies $|f'(u)-f'(v)|<\varepsilon$ for all $u,v\in[a,b]$.  By the mean value theorem, for $0<|t-x|<\delta$ there is a point $c$ between $x$ and $t$ such that
\[
 \frac{f(t)-f(x)}{t-x}=f'(c).
\]
Since $|c-x|\le|t-x|<\delta$, the required inequality follows.  The same $\delta$ works for every $x,t\in[a,b]$.
...
\begin{align*}
f(x)=\left\{\begin{array}{ll}
\left\vert x\right\vert^a\sin\left(\left\vert x\right\vert^{-c}\right)&\mbox{(if $x\ne 0$)}\vspace{0.08in}\\
0&\mbox{(if $x=0$)}
\end{array}\right.
\end{align*}
...
\smallskip
\noindent\textbf{Solution.}\quad
(c)--(g) For $x\ne0$, differentiation shows that $f'(x)$ is a sum of terms of orders $|x|^{a-1}$ and $|x|^{a-c-1}$, while $f''(x)$ is a sum of terms of orders $|x|^{a-2}$, $|x|^{a-c-2}$, and $|x|^{a-2c-2}$.  The oscillatory factors are bounded.  Thus the dominant first-derivative term is bounded exactly when $a\ge1+c$, and tends to zero exactly when $a>1+c$; these give (c) and (d), using $f'(0)=0$.  Likewise the difference quotient for $f'$ at zero tends to zero exactly when $a>2+c$, proving (e).  The dominant second-derivative term has size $|x|^{a-2-2c}$, so it is bounded exactly when $a\ge2+2c$ and tends to zero exactly when $a>2+2c$, proving (f) and (g).  Necessity at each borderline follows by choosing sequences on which the corresponding sine or cosine factor equals $1$ in absolute value.
...
\end{align*}
and since $N_1^2\le 4N_0N_2$ [by part (i)], we have $p^2<4M_0M_2$. Since $p$ may increase to $M_1$, we conclude that $M_1^2\le 4M_0M_2$.
...
(ii) On a right-hand interval, every $c\ge0$ gives a solution
\[
 y_c(x)=\begin{cases}0,&0\le x\le c,\\(x-c)^2/4,&x\ge c.\end{cases}
\]
The two formulas and their first derivatives agree at $c$, so $y_c'=\sqrt{y_c}$ everywhere.  Conversely, because every solution is nondecreasing and nonnegative, its zero set is an initial interval $[0,c]$ (possibly all of the domain).  Where the solution is positive, $2\sqrt y$ has derivative $1$, so $2\sqrt{y(x)}=x-c$ and hence $y(x)=(x-c)^2/4$.  These delayed solutions, together with the identically zero solution, are all solutions.
