...
\title{Introduction to Smooth Manifolds: Integration on Manifolds Solutions}
\begin{document}
\maketitle
...
\begin{aligned}
  ...
  &  \beta(X) = X \mathbin{\lrcorner} dV_g
\end{aligned}
\]
...
\[
\beta(X) = X \mathbin{\lrcorner} dV_g = X^{\perp} \mathbin{\lrcorner} dV_g + X^T \mathbin{\lrcorner} dV_g
\]
...
\[
i^*_S{ (X^{\perp} \mathbin{\lrcorner} dV_g) }  = \langle X, N \rangle_g i^*_S{ (N \mathbin{\lrcorner} dV_g) } = \langle X, N \rangle_g dV_{ \widetilde{g}}
\]

If $X_1 \dots X_{n-1}$ any vectors tangent to $S$

\[
(X^T \mathbin{\lrcorner} dV_g){ (X_1 \dots X_{n-1} ) } = dV_g{ (X^T, X_1 \dots X_{n-1} ) } = 0
\]
