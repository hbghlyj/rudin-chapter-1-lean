...
Thus by the claim and by Theorem 4.14, $g(E)$ is compact, i.e., $G(f)$ is compact (since $G(f)=g(E)$). Conversely, suppose $f$ is not continuous at some point $x\in E$, then there is a sequence $\{p_n\}$ in $E$ such that $\lim_{n\to\infty}p_n=x$ but $\lim_{n\to\infty}f(p_n)\ne f(x)$. If no subsequence of $\{f(p_n)\}$ converges, then no subsequence of $\{(p_n,f(p_n))\}$ in $G(f)$ converges, and then $G(f)$ is not compact [by Theorem 3.6(a)]. If there is a subsequence $\{(p_{n_k},f(p_{n_k}))\}$ in $G(f)$ that converges to $(x,y)$ for some $y\in Y$ with $y\ne f(x)$. Then $(x,y)$ is a limit point of $G(f)$, but $(x,y)\notin G(f)$. So $G(f)$ is not closed, and then $G(f)$ is not compact.
...
Case 4: If $x,y\notin E$, then by the definition of $g$ and since $E$ is dense in $X$, we can choose points $z_1,z_2\in E$ such that $d(x,z_1)<\frac{\delta}{3}$, $d(x,z_2)<\frac{\delta}{3}$, and $\left\vert f(z_1)-g(x)\right\vert<\varepsilon$, $\left\vert f(z_2)-g(y)\right\vert<\varepsilon$. Then
\begin{align*}
d(z_1,z_2)
...
<2\varepsilon
\end{align*}
If $q\notin E$, then since $E$ is dense in $X$, the set $V_{2N}(q)\ne\varnothing$, and there exists a point $p\in V_{2N}(q)\subset V_N(p)$. It follows that $g(q)\in\overline{f( V_{N}(p))}$, and that $\left\vert f(p)-g(q)\right\vert<\varepsilon$. On the other hand, since $d(p_0,q)<\frac{1}{2N}$, we have
\begin{align*}
...
&=f(x)+\sup E_2\cdot\lim_{t\in(x,b),t\to x}(t-x) \\
&=f(x)
\end{align*}
...
+\frac{1}{2}\left[\frac{t}{2^n}f(x)+\left(1-\frac{t}{2^n}f(y)\right)\right] \\
