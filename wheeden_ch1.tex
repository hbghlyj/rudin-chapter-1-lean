...
\smallskip
\noindent\textbf{Solution.}\quad
(q) Let $\{G_\alpha\}$ be an open cover of $TE$.  By continuity of $T:E\to T(E)$, each $E\cap T^{-1}G_\alpha$ is open in the relative topology on $E$: for $\mathbf{x}$ in this preimage, continuity supplies $\delta>0$ such that every $\mathbf y\in E\cap B(\mathbf x,\delta)$ is mapped into $G_\alpha$.
...
By using the above two claims, the collection $\{E\cap T^{-1}G_\alpha\}$ forms a relatively open cover of $E$. Since $E$ is compact, there exist a finite number of indices $\alpha_1,\alpha_2,\ldots,\alpha_m$, so that
$$
...
\end{align*}
The result follows by taking $\varepsilon\to 0$. For (c), if $a=0$, the desired reverse inequality follows from nonnegativity.  If $a>0$, choose $0<\varepsilon<a$. Again, there is an integer $J$ such that $a_k>a-\varepsilon$ for all $k\ge J$, and then $a_kb_k>(a-\varepsilon)b_k$ for all $k\ge J$. It follows that for $j\ge J$,
\begin{align*}
\sup_{k\ge j}(a_kb_k)
...
For the second, given $\varepsilon>0$. Since $f_k\to f$ uniformly, if $|I|=0$ the assertion is immediate. Otherwise, there is an integer $K$ such that $f_k(\mathbf{x})-\varepsilon/|I|<f(\mathbf{x})<f_k(\mathbf{x})+\varepsilon/|I|$ for $k\ge K$ and for $\mathbf{x}\in I$. It follows that each upper sum changes by at most
$(\varepsilon/|I|)\sum_j |I_j|=\varepsilon$.  Taking infima over partitions therefore gives
\begin{align*}
