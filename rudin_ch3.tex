...
\smallskip
\noindent\textbf{Solution.}\quad
(d) Put $s=\sqrt{\alpha}$ and $\varepsilon_n=|x_n-s|$.  The exact error identity is
\begin{align*}
\varepsilon_{n+1}
...
&=\frac{s-1}{1+x_n}\varepsilon_n.
\end{align*}
Since $x_n\to s$, the ratio $\varepsilon_{n+1}/\varepsilon_n$ tends to
\[
 \frac{s-1}{s+1}<1.
\]
Thus this iteration converges linearly with asymptotic factor
$(\sqrt\alpha-1)/(\sqrt\alpha+1)$.  By contrast, the Newton iteration
in Exercise 16 has an error bounded by a constant times the square of
the preceding error, and hence converges quadratically.
...
Since $X\ne\varnothing$, we begin with some point $x_0\in X$ and given $r>0$. Since $G_1$ is dense and open, the set $N_r(x_0)\cap G_1$ is nonempty and open. Choose $x_1\in N_r(x_0)\cap G_1$ and there exists a neighborhood $E_1$ of $x_1$ such that
\begin{align*}
...
The completion is isometric to $\mathbb{R}$.  Indeed, send the
equivalence class of a rational Cauchy sequence $(p_n)$ to its real
limit.  Equivalent sequences have mutual distance tending to zero and
therefore have the same real limit, so this map is well defined.  For
classes represented by $(p_n)$ and $(q_n)$, continuity of absolute value
gives
\[
 \lim_n |p_n-q_n|=\left|\lim_n p_n-\lim_n q_n\right|,
\]
so the map is an isometry.  Finally every real number is the limit of a
rational Cauchy sequence, by density of $\mathbb{Q}$ in $\mathbb{R}$;
thus the isometry is onto.
