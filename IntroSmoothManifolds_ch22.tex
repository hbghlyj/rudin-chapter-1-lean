...
\title{Introduction to Smooth Manifolds: Symplectic Manifolds Solutions}
\begin{document}
\maketitle
...
\mathcal{L}_VH = VH = 0
\]
\item[(c)] Let $\gamma:I\to M$ be a trajectory, so
$\dot\gamma=X_H\circ\gamma$, and let $\theta_s$ be a time-$s$ map of
the flow of an infinitesimal symmetry $V$.  Since $\theta_s$ preserves
both $\omega$ and $H$, it preserves the Hamiltonian vector field:
\[
  (\theta_s)_*X_H=X_H.
\]
Indeed, applying $\theta_s^*$ to
$\iota_{X_H}\omega=dH$ gives the same defining equation for the
pushforward field, and nondegeneracy of $\omega$ gives uniqueness.
Consequently
\[
 \frac{d}{dt}(\theta_s\circ\gamma)(t)
   =(\theta_s)_*\dot\gamma(t)
   =(\theta_s)_*X_H(\gamma(t))
   =X_H((\theta_s\circ\gamma)(t)).
\]
Thus $\theta_s\circ\gamma$ is again a trajectory of the Hamiltonian
system.
\end{enumerate}
\end{proof}
