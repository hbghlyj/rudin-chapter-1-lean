...
Every subset $A\subset X$ is open: for each $p\in A$, the ball of
radius $1/2$ about $p$ is the singleton $\{p\}$ and is contained in
$A$.  Since the complement of every subset is likewise open, every
subset is also closed.

Every finite subset is compact, because an open cover can be reduced to
one chosen member for each of its finitely many points.  Conversely, if
$A$ is infinite, then the family of singleton sets $\{\{p\}:p\in A\}$
is an open cover of $A$ with no finite subcover.  Thus the compact
subsets are exactly the finite subsets.
...
Yes. Enumerate the rationals as $\{r_1,r_2,\ldots\}$ and start
with a nondegenerate closed interval $E_0$ having irrational endpoints.
Inductively suppose that $E_n$ is a finite disjoint union of nondegenerate
closed intervals, has irrational endpoints, and avoids
$r_1,\ldots,r_n$.  From the interior of each component $[a,b]$, remove
an open interval $(c,d)$ whose endpoints are irrational and are chosen
close enough to the one-third and two-thirds points that both retained
intervals have length less than $(b-a)/2$.  Density of the irrationals
makes this choice possible; no claim that arithmetic means of irrational
numbers remain irrational is needed.  If $r_{n+1}$ lies in one of the
retained intervals, remove from the interior of that interval a further
open interval with irrational endpoints which contains $r_{n+1}$ (it
cannot be an endpoint, since it is rational).  Call the remaining finite
union $E_{n+1}$.  Every component is nondegenerate, has irrational
endpoints, and has length less than half the length of its parent
component; moreover $E_{n+1}$ avoids $r_1,\ldots,r_{n+1}$.  Hence the resulting set
$P=\bigcap_{n=0}^\infty E_n$ contains no rational number.
Since $E_n$ is closed for each $n\ge 0$, by Theorem 2.24(b), $P$ is closed. Moreover, since $E_0\supset E_1\supset E_2\supset \cdots$, each $E_n$ is bounded (by $E_0$), i.e., each $E_n$ is actually a (nonempty) compact set. By Theorem 2.36, $P\ne\varnothing$. Finally, let $L$ be the length of $E_0$ and let $x\in P$.  If $[a,b]$ is the component of $E_n$ containing $x$, then $b-a<L2^{-n}$.  Every endpoint created by the construction is retained at all later stages, so $a,b\in P$.  Choose $y=a$ unless $x=a$, in which case choose $y=b$.  Then $y\in P$, $y\ne x$, and
\begin{align*}
\left\vert y-x\right\vert<L2^{-n}.
\end{align*}
As this tends to zero, $x$ is a limit point of $P$.  Hence $P$ is perfect.
