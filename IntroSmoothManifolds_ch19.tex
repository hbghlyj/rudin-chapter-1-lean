...
\title{Introduction to Smooth Manifolds: Distributions and Foliations Solutions}
\begin{document}
\maketitle
...
\problemhead{19-3}
Let $\omega\in\Omega^1(M)$ be nowhere zero on an open set $U$.  An
\emph{integrating factor} near $p\in U$ is a positive smooth function
$\mu$ such that $\mu\omega=df$ locally for some smooth function $f$.

\begin{enumerate}
\item[(a)] Such a local integrating factor exists near every point of
$U$ if and only if $\omega\wedge d\omega=0$ on $U$.

If $\mu\omega=df$, then
\[
  0=d^2f=d\mu\wedge\omega+\mu d\omega.
\]
Wedging with $\omega$ and using $\omega\wedge\omega=0$ gives
$\mu\,d\omega\wedge\omega=0$, hence $\omega\wedge d\omega=0$.

Conversely, let $D=\ker\omega$.  For vector fields $X,Y$ tangent to
$D$, the identity
\[
  d\omega(X,Y)=X(\omega(Y))-Y(\omega(X))-\omega([X,Y]).
\]
together with $\omega\wedge d\omega=0$ and the fact that $\omega$ is
nowhere zero shows that $d\omega(X,Y)=0$.  Thus $D$ is involutive.
The Frobenius theorem yields, near each $p\in U$, a submersion $f$ whose
level hypersurfaces are the integral manifolds of $D$.  Therefore
$\ker df=\ker\omega$, so $\omega=h\,df$ for a nowhere-zero smooth
function $h$.  After replacing $f$ by $-f$ if necessary and shrinking
the neighborhood, assume $h>0$.  Then $\mu=h^{-1}>0$ and
$\mu\omega=df$.

\item[(b)] If $\dim M=2$, every three-form vanishes, so
$\omega\wedge d\omega=0$.  Part (a) therefore supplies a local
integrating factor near every point where $\omega$ is nonzero.
\end{enumerate}
