...
\title{Introduction to Smooth Manifolds: Smooth Maps Solutions}
\begin{document}
\maketitle
\section{Smooth Maps}
% 02smoothmaps.tex
...
\begin{proof}
Choose a locally finite refinement $(V_j)_{j\in J}$ of the given cover
such that each $\overline{V_j}$ lies in some $X_{\alpha(j)}$, and choose
open sets $W_j$ with $\overline{W_j}\subset V_j$ that still cover $M$.
For every $j$, the smooth bump-function lemma provides
$\rho_j\in C^\infty(M,[0,1])$ satisfying $\rho_j=1$ on
$\overline{W_j}$ and $\operatorname{supp}\rho_j\subset V_j$.
Local finiteness makes
\[
  \rho=\sum_{j\in J}\rho_j
\]
a smooth function.  Since the $W_j$ cover $M$, at least one summand is
one at every point, so $\rho>0$.  Hence $\psi_j=\rho_j/\rho$ is smooth,
the family $(\psi_j)$ is locally finite, and
$\sum_j\psi_j=1$.  Moreover,
$\operatorname{supp}\psi_j\subset V_j\subset X_{\alpha(j)}$.
Finally, for each $\alpha\in A$, set
$\varphi_\alpha=\sum_{\alpha(j)=\alpha}\psi_j$.  These locally finite
sums form the required smooth partition of unity subordinate to the
original indexed cover.
\end{proof}
